...
export type State = {
  tasks: Task[];
  draggedTask: string | null; // id of the dragged task
};
...
export const useTaskStore = create(persist<State & Actions>(
  (set) => ({
    tasks: [],
    draggedTask: null,
    ...
    })) // updating task with the specific id and returning other tasks unchanged
  }),
  {
    name: 'task-store', // name of the item in the storage
